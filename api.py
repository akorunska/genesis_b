...
class ChainBlock(Resource):
    def get(self):
        ...
        if args['height']:
            if (args['height'] < len(list)):
                block = list[args['height']]
            else:
                code = 404
        elif args['hash']:
            block = next((x for x in list if x.hash_value == args['hash']), "")
            if not block:
                code = 404
        else:
            block = blocks.get_last_block()

        json_repr =  json.dumps(block.__dict__) if code == 200 else ""
...
api.add_resource(ProduceBlock, '/produce-block', methods=['POST'])
